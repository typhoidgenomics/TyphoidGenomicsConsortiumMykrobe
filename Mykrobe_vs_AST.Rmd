...
cip_mic <- amr_ast %>% 
  ...
  mutate(qnr = str_count(ciprofloxacin, "qnr")) %>%
...
upset_cip_mic_UK <- cip_mic %>%
  ...
  upset(name='Genotype', themes=upset_default_themes(text=element_text(size=8)),
      intersect=c( "qnr", "GyrB464", "ParC84","ParC80", "GyrA87","GyrA83","wt_QRDR"),
      sort_sets=F,
      intersections=list(
        'wt_QRDR',
...
ggsave("Cip_MIC.pdf", width=9, height=4.5)
ggsave("CIP_MIC.png", width=9, height=4.5, units="in")
```
