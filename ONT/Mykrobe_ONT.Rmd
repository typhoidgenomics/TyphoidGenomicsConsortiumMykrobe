---
title: "Global Typhoid Genomics Consortium - Mykrobe - ONT comparison"
...
ont <- read_csv("../Supplementary_Table_6.csv")
...
```
# numbers for text - genotype
```{r}
#genotypes
genotype <- ont.typhi %>% select(ONT_genotype, genotype) %>%  
  mutate(Genotype_match = if_else(ONT_genotype == genotype, "Genotype Match", "Mismatch")) 

genotype %>% group_by(pick(Genotype_match)) %>% count()

genotype %>% filter(Genotype_match == "Mismatch")
```
...
#chloramphenicol
ont.typhi %>% select(ONT_chloramphenicol, chloramphenicol)  %>% group_by(pick(everything())) %>% count()
...
#sulfonamides
ont.typhi %>% select(ONT_sulfonamides, sulfonamides)  %>% group_by(pick(everything())) %>% count()

#trimethoprim
ont.typhi %>% select(ONT_trimethoprim, trimethoprim)  %>% group_by(pick(everything())) %>% count()
