...
amr <- myk_data %>% left_join(valid_data, by = "Sanger_ID")
...
# Fig 2 - Sankey for MDR overview
```{r}
amr_mdr <-  amr %>%  mutate(MYK_MDR =if_else(MYK_ampicillin != "S"   & MYK_chloramphenicol != "S" & `MYK_trimethoprim-sulfamethoxazole` != "S", "MDR", "S"))
amr_mdr %>% group_by(MYK_MDR,PW_MDR)  %>% count()

amr_mdr<- amr_mdr %>% filter(MYK_MDR != "S" | PW_MDR != "-" )
amr_mdr$MDR <- "MDR"

amr_mdr %>% group_by(MYK_sulfonamides,PW_sul_any)  %>% count()
amr_mdr %>% group_by(MYK_trimethoprim,PW_dfra_any)  %>% count()

amr_mdr_genes <- amr_mdr %>% 
  #sulfonamides 
  mutate('PW_sulfonamides' = case_when(
  PW_sul1 & PW_sul2 == 1 ~ "R: sul1;sul2", 
  PW_sul1 == 1 & PW_sul2 == 0 ~ "R: sul1",
  PW_sul1 == 0 & PW_sul2 == 1 ~ "R: sul2",
  PW_sul1  == 0 & PW_sul2 == 0 ~ "S",
  )
)  %>% 
  #trimethoprim 
  mutate('PW_trimethoprim' = case_when(
  PW_dfrA14 & PW_dfrA7 == 1 ~ "R: dfrA14;dfrA7 ", 
  PW_dfrA7 == 1 & PW_dfrA14 == 0 ~ "R: dfrA7",
  PW_dfrA5 == 1  ~ "R: dfrA5",
  PW_dfrA15 == 1  ~ "R: dfrA15",
  PW_dfrA17  == 1  ~ "R: dfrA17",
  PW_dfra_any == 0 ~ "S",
  )
) %>% 
  mutate(PW_ampicillin= ifelse(`PW_blaTEM-1D` == 1, "R: blaTEM-1D", "S")) %>%
  mutate(PW_chloramphenicol= ifelse(`PW_catA1` == 1, "R: catA1 ", "S")) %>%
  mutate(`PW_MDR`=gsub("-", "S", PW_MDR)) 

mdr_long <- amr_mdr_genes %>% make_long(MDR,MYK_MDR, PW_MDR, MYK_ampicillin,`PW_ampicillin`,MYK_chloramphenicol, PW_chloramphenicol,MYK_trimethoprim,PW_trimethoprim, MYK_sulfonamides, PW_sulfonamides)

# Chart - MDR Resistance only
sankey_mdr <- ggplot(mdr_long, aes(x = x
                     , next_x = next_x
                     , node = node
                     , next_node = next_node
                     , fill = factor(node)
...
sankey_mdr <- sankey_mdr +geom_sankey(flow.alpha = 0.5
                      , node.color = "black"
                      ,show.legend = FALSE)
sankey_mdr <- sankey_mdr +geom_sankey_label(size = 2, color = "black", fill= "white", hjust = -0.15)
sankey_mdr <- sankey_mdr +  theme_bw()
sankey_mdr <- sankey_mdr + theme(legend.position = "bottom") +  theme(axis.title = element_blank()
                  , axis.text.y = element_blank()
                  , axis.ticks = element_blank()  
                  ,axis.text.x  = element_text(angle=90, vjust=0.5, size=8)
                  , panel.grid = element_blank()) +  theme(panel.border = element_blank())
sankey_mdr <- sankey_mdr + scale_fill_viridis_d(drop = FALSE)
#sankey_mdr <- sankey_mdr + labs(title = "MDR Resistance \nn = 3383")
sankey_mdr <- sankey_mdr + labs(fill = 'Nodes')

```

# write Fig2 to PDF & PNG
``` {r}
sankey_mdr

ggsave("Figure2_Sankey_MDR.pdf", width=10, height=5)
ggsave("Figure2_Sankey_MDR.png", width=10, height=5, units="in")
```
