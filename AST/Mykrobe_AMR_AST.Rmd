...
# clean AST analysis
``` {r files}
amr_ast <- read_csv("../Supplementary_Table_3.csv")
```
...
# numbers for text - SCT
``` {r}
amr_ast %>% filter(dataset=="CDC" & `SXT Result`=="R" & sulfamethoxazole=="S") %>% select(sulfamethoxazole, `SXT Measure`, TGC_sangerlane)

amr_ast %>% filter(dataset=="UKHSA" & `SXT Result`=="R" & sulfamethoxazole=="S") %>% select(sulfamethoxazole, trimethoprim, sulfonamides, `SXT Measure`, TGC_sangerlane)
...
cip_mic <- amr_ast %>% 
  ...
  mutate(cip_MIC=replace(cip_MIC, cip_MIC=="6.4000000000000001E-2", 0.06)) %>%
  mutate(cip_MIC=replace(cip_MIC, cip_MIC=="1.4999999999999999E-2", 0.015)) %>%
      ...
      base_annotations=list( # add stacked barplot
        ...
        aes=aes(x=intersection, fill=cip_pheno),
          geom=list(
            ...
            scale_fill_manual(values = c("IndianRed", "grey")),
      ...
      base_annotations=list( # add stacked barplot
        ...
        aes=aes(x=intersection, fill=cip_pheno),
          geom=list(
            geom_bar(stat='count', position='fill'), 
            theme(legend.position = "none", axis.title=element_text(size=8)),
            scale_fill_manual(values = c("grey", "#EEBB74", "IndianRed")),
            scale_y_continuous(labels=scales::percent_format()),
            geom_text(aes(label=..count..), stat="count", position=position_fill(vjust = .5), angle = 45, size=2))
        )
      ), 
      annotations= list( # add violin plot with MIC data
        'MIC measurement (mg/L)'=(
            ggplot(mapping=aes(y=as.numeric(cip_MIC))) +
              ...
              geom_violin()+ 
              geom_count(aes(colour = cip_pheno))+
              scale_color_manual(values = c("grey", "#EEBB74", "IndianRed"))+
...
ggsave("Fig3_Cip_MIC.pdf", width=9, height=5)
ggsave("Fig3_CIP_MIC.png", width=9, height=5, units="in")
```
...
ggsave("Fig4_SEAP_AST.pdf", width=9, height=5)
ggsave("Fig4_SEAP_AST.png", width=9, height=5, units="in")
```
